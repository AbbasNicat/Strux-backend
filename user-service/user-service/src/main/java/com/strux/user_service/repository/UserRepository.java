...
public interface UserRepository extends JpaRepository<User, String> {
    ...
    @Query("SELECT u FROM User u WHERE u.role = :role AND u.status = :status " +
            "AND (:specialty IS NULL OR u.workerProfile.specialty = :specialty) " +
            "AND (:city IS NULL OR LOWER(u.city) = LOWER(:city)) " +
            "AND (:isAvailable IS NULL OR u.isAvailable = :isAvailable) " +
            "AND u.workerProfile.rating >= :minRating")
    ...
    List<User> findByCompanyIdAndStatus(
            String companyId,
            UserStatus status
    );

    Page<User> findByCompanyIdAndStatus(
            String companyId,
            UserStatus status,
            Pageable pageable
    );

    @Query("SELECT u FROM User u WHERE u.workerProfile.specialty = :specialty " +
            "AND u.status = :status " +
            "ORDER BY u.workerProfile.rating DESC")
