...
public class AuthService {
    ...
    public LoginResponse login(LoginRequest request, String ipAddress, String userAgent,
        ...
        try {
            ...
            // 2FA requirement
            if (require2FA || securityContext.isSuspicious()) {
    ...
    public LoginResponse verify2FA(String twoFAToken, String code, String ipAddress, String userAgent) {
        try {
            ...
            String userId = twoFactorAuthService.getUserIdFromToken(twoFAToken);
            ...
            if (tempData == null) {
                ...
                String email = keycloakAdminService.getUserById(userId).getEmail();
                LoginResponse tokenResponse = getKeycloakTokenForGoogleUser(userId, email, null);

                if (tokenResponse.getRefreshToken() != null) {
                    manageUserSessions(userId, tokenResponse.getRefreshToken());
                }
                updateLastLoginTime(userId, ipAddress);
    ...
    private SecurityContext analyzeSecurity(String userId, String ipAddress, String userAgent, String deviceFingerprint) {
        ...
        if (lastLoginTime != null) {
            ...
            if (minutesSinceLastLogin < 30) {
                String lastLoginLocation = geoLocationService.getLastKnownLocation(userId);
                if (lastLoginLocation != null && !lastLoginLocation.equals(currentLocation)) {
                    int distance = geoLocationService.calculateDistance(lastLoginLocation, currentLocation);
    ...
    private void manageUserSessions(String userId, String refreshToken) {
        ...
        try {
            ...
            log.info("Session managed for user: {}", userId);
        } catch (Exception e) {
            log.error("Session management error for user {}: {}", userId, e.getMessage());
    ...
    private void checkAdvancedRateLimit(String ipAddress, String email) {
        ...
        String luaScript =
                "local ipReqs = redis.call('get', KEYS[1]) or '0' " +
                        "local emailReqs = redis.call('get', KEYS[2]) or '0' " +
                        "if tonumber(ipReqs) >= tonumber(ARGV[1]) then " +
                        "  return 1 " +
                        "end " +
                        "if tonumber(emailReqs) >= tonumber(ARGV[2]) then " +
                        "  return 2 " +
                        "end " +
        ...
        try {
            Long result = redisTemplate.execute(
                    RedisScript.of(luaScript, Long.class),
                    Arrays.asList(ipKey, emailKey),
                    String.valueOf(MAX_REQUESTS_PER_MINUTE),
                    String.valueOf(MAX_EMAIL_REQUESTS_PER_MINUTE)
            );
            ...
            // Don't throw - fail open for rate limiting
        }
    }
    ...
    private LocalDateTime getLastLoginTime(String userId) {
        try {
            ...
            return timestamp != null ? (LocalDateTime) timestamp : null;
        } catch (Exception e) {
    ...
    private TempSessionData getTempSessionData(String twoFAToken) {
        try {
            String key = TEMP_SESSION_PREFIX + twoFAToken;
            return (TempSessionData) redisTemplate.opsForValue().get(key);
        } catch (Exception e) {
            log.error("Get temp session error: {}", e.getMessage());
            return null;
        }
    }
                ...
                if (rolesNode.isArray()) {
                    for (JsonNode roleNode : rolesNode) {
                        ...
                        switch (keycloakRole) {
                            ...
                            case "MANAGER": return "PROJECT_MANAGER";    // Mapping
                            case "OWNER": return "UNIT_OWNER";           // Mapping
